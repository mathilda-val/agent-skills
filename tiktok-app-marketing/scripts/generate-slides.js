...
async function generateGoogle(prompt, outPath) {
  const googleModel = model || 'imagen-4.0-generate-001';
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${googleModel}:predict?key=${apiKey}`, {
    method: 'POST',
  ...
  if (!data.predictions?.[0]?.bytesBase64Encoded) {
    throw new Error('No image data in Google Imagen response: ' + JSON.stringify(Object.keys(data)));
  }
  fs.writeFileSync(outPath, Buffer.from(data.predictions[0].bytesBase64Encoded, 'base64'));
}
